...
async function checkAndDisplayExistingOcorrencias(telefone) {
    try {
...
async function showMultipleOcorrenciasAlert(matchingOcorrencias) {
    ...
    // Separate occurrences by status
    const encerradas = [];
    ...
    // Action buttons based on what we found
    let buttonsHTML = '<div style="display: flex; flex-wrap: wrap; gap: 10px; margin-top: 10px;">';
    
    // Show resgatar if there are closed occurrences
    if (encerradas.length > 0) {
        const [key, ocorrencia] = encerradas[0]; // Most recent closed
    ...
    // Show reiterar options if there are pending or in-progress occurrences
    if (pendentesOuEmAtendimento.length > 0) {
        const [key, ocorrencia] = pendentesOuEmAtendimento[0]; // Most recent pending/in-progress
        buttonsHTML += `
...
async function showOcorrenciaAlert(key, ocorrencia) {
    // This function is now replaced by showMultipleOcorrenciasAlert
    await showMultipleOcorrenciasAlert([[key, ocorrencia]]);
}

function setupOcorrenciaHandlers(key, ocorrencia, alertDiv, telefoneInput, isEncerrada) {
    // This function is now replaced by setupMultipleOcorrenciasHandlers
}
